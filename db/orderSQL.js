var orderSQL={
	...
	queryPostList:'select wp.ID, DATE_FORMAT(wp.DateTime,'%Y-%m-%d %H:%i:%s'),wp.IsBalance,wp.IsPost,wp.BalanceTime,Wp.PostTime,wu.`UserName`,wu.`WeiXinName` from WeiPack wp left join `WeiUsers` wu on wp.UserID=wu.`ID`',
