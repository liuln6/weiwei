var orderSQL={
	...
	queryPostList:'select wp.ID,wp.`UserID`, DATE_FORMAT(wp.DateTime,"%Y-%m-%d") DateTime,wp.IsBalance,wp.IsPost,wp.BalanceTime,wp.`PostTime`,wu.`UserName`,wu.`WeiXinName` from WeiPack wp left join `WeiUsers` wu on wp.UserID=wu.`ID`',
