var orderSQL={
	...
	queryPostList:'select wp.ID,wp.`UserID`, DATE_FORMAT(wp.DateTime,"%Y-%m-%d") DateTime,wp.IsBalance,wp.IsPost,wp.BalanceTime,wp.`PostTime`,wp.TotalNumber,wp.TotalOrderNumber,wp.TotalPrice,wp.TotalPriceActive,wu.`UserName`,wu.`WeiXinName` from WeiPack wp left join `WeiUsers` wu on wp.UserID=wu.`ID`',
	queryPostInfo:'select wu.ID,wu.WeiXinID,wu.WeiXinName,wu.UserName,wp.`IsPost`,wp.`IsBalance`,DATE_FORMAT(wp.PostTime,"%Y-%m-%d") PostTime,DATE_FORMAT(wp.BalanceTime,"%Y-%m-%d") BalanceTime,TotalPriceActive from `WeiUsers` wu left join `WeiPack` wp on wu.ID=wp.`UserID` and wp.`ID`=? where wu.ID=?; select ID,Address,Name,Phone,ZipCode from `WeiUsersAddress` where `UserID`=?; select wo.*,wp.`ProductName`,wpt.`Remark` TypeName from `WeiOrder` wo left join `WeiProduct` wp on wo.`ProductID`=wp.ID left join `WeiProductImageType` wpt on wo.`TypeID`=wpt.ID where wo.ID in( select `OrderID` from `WeiOrderPack` where `PackID`=?)',
